import { M as Observable, f as ApolloLink, l as CombinedGraphQLErrors, n as graphQLResultHasProtocolErrors, r as toErrorLike, t as PROTOCOL_ERRORS_SYMBOL } from "./errors-DYsTXaAd.js";
				...
				try {
					sub = forward(operation).subscribe({
						next: (result) => {
							...
							if (errors) retriedResult = errorHandler({
								error: new CombinedGraphQLErrors(result, errors),
								result,
								operation,
								forward
							});
							else if (graphQLResultHasProtocolErrors(result)) retriedResult = errorHandler({
							...
							if (!retriedSub) observer.next(result);
						},
						error: (error) => {
							...
							retriedSub = retriedResult?.subscribe(observer);
							if (!retriedSub) observer.error(error);
						},
						complete: () => {
							if (!retriedResult) observer.complete();
						}
					});
				} catch (e) {
					errorHandler({
